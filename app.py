"""اسکنر سیگنال کریپتو — خروجی روایی و ساده (موبایل‌محور).
...
# نقشه تایم‌فریم بالاتر (فقط تکمیل گزینه‌های UI؛ منطق تأیید HTF تغییر نکرده)
...
.sigcard .hdr { font-size:1.1rem; font-weight:800; margin-bottom:8px; }
...
def tf_box(key):
    """هر پارامتر با انتخاب تایم‌فریم جفت شده؛ همه به یک تایم‌فریم اسکن متصل‌اند."""
...
stream = st.container()
for n in ss["cards"]:
    stream.markdown(narrative_html(n), unsafe_allow_html=True)
...
if run:
    ...
    for i in range(start, end):
        ...
        if not res["signal"]:
            continue
        n = build_narrative(r["symbol"], res, CFG["quote"])
        ss["cards"].append(n)
        stream.markdown(narrative_html(n), unsafe_allow_html=True)
        found += 1
        if stop_first:
